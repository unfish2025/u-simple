<template>
	<view ref="uUpload" class="u-base u-border-box u-upload-image">
		...
		<slot :chooseFile="chooseFile" :list="value" v-if="computedIsShow">
			<view
				class="u-upload-image-add-container u-border-box"
				:class="{ 'u-upload-image-is-disabled': !allowOperate, 'u-upload-image-is-active': allowOperate }"
				@click="chooseFile"
			>
...
.u-upload-image {
	--u-upload-image-width: 160rpx;
	--u-upload-image-height: 160rpx;
	--u-upload-image-add-icon-font-size: 60rpx;
	--u-upload-image-gap: 10rpx;
	...
	display: flex;
	flex-wrap: wrap;
	gap: var(--u-upload-image-gap);
}

.u-upload-image-add-container {
	display: flex;
	justify-content: center;
	align-items: center;
	width: var(--u-upload-image-width);
	height: var(--u-upload-image-height);
	border: 1px dashed var(--u-placeholder-color);
	border-radius: var(--u-upload-border-radius);
	font-size: var(--u-upload-image-add-icon-font-size);
	color: var(--u-placeholder-color);
	cursor: pointer;
	transition: color 0.2s, border-color 0.2s;

	&.u-upload-image-is-active:active {
		border-color: var(--u-primary-color);
		color: var(--u-primary-color);
	}

	&.u-upload-image-is-disabled {
		opacity: 0.6;
		cursor: not-allowed;
...
.u-upload-image-item-content-image {
	width: 100%;
	height: 100%;
	display: block;
}

.u-upload-image-remove-icon {
	position: absolute;
	top: 0;
	right: 0;
	display: flex;
